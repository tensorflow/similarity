...
from tensorflow.keras.layers import Convolution1D, Dense, Input, add
...
def residual_block(x,
    ...
    x = SpatialDropout1D(
        dropout_rate,
        name=name + '_spatial_dropout1d_%d_s%d_%f' % (i, s, dropout_rate))(x)
...
class TCN:
    ...
    def __init__(self,
        ...
        if not isinstance(nb_filters, int):
            ...
            print(
                'Second solution is to pip install keras-tcn==2.1.2 to downgrade.'
            )
            raise Exception()
...
def compiled_tcn(
    ...
    x = TCN(nb_filters, kernel_size, nb_stacks, dilations, activation, padding,

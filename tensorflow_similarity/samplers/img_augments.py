...
def _convert_translation_to_transform(translations: tf.Tensor) -> tf.Tensor:
    ...
    """
    if not tf.is_tensor(translations):
        translations = tf.convert_to_tensor(np.array(translations))
        ...
        for policy in self.policies:
            ...
            for policy_info in policy:
                ...
                assert_ranges.append(
                    tf.Assert(tf.less_equal(prob, 1.0), [prob])
                )
                assert_ranges.append(
        ...
        for _ in range(count):
            ...
            for _ in range(self._trials):
                if not is_trial_successfull:
                    ...
                    half_height = tf.cast(
                        tf.math.round(
                            tf.math.sqrt(erase_area * aspect_ratio) / 2
                        ),
                        dtype=tf.int32,
                    )
                    half_width = tf.cast(
                    ...
                    if (
                        2 * half_height < image_height
                        and 2 * half_width < image_width
                    ):
    ...
    def _cutmix(
        ...
        images = tf.map_fn(
            ...
            fn_output_signature=tf.TensorSpec(
                images.shape[1:], dtype=tf.float32
            ),
        )

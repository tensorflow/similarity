    ...
    with tf.name_scope("distort_color"):
        ...
        def apply_transform(i, x, brightness, contrast, saturation, hue):
            ...
            if brightness != 0 and i == 0:
                ...
                x = tf.image.random_contrast(x, lower=1 - contrast,
                                             upper=1 + contrast)
        ...
        for i in range(4):
            image = apply_transform(i, image, brightness, contrast,
                                    saturation, hue)
...
def batch_random_blur(images_list, height, width, blur_probability=0.5):
    ...
    def generate_selector(p, bsz):
        shape = [bsz, 1, 1, 1]
        selector = tf.less(tf.random.uniform(shape, 0, 1, dtype=tf.float32), p)
...
def simclr_eval_augmentation(image, height, width,
                             crop=True, crop_proportion=0.875):
...
class SimCLRAugmenter(Augmenter):
    ...
    def augment(self,
        ...
        with tf.device("/cpu:0"):
            ...
            for _ in range(num_views):
                ...
                view = tf.map_fn(self.augment_img, inputs,
                                 parallel_iterations=self.num_cpu)
                views.append(view)
        return views

    def _train_augment_img(self, img):
        return simclr_training_augmentation(img, self.height,
                                            self.width,
                                            self.color_distort,
                                            self.jitter_stength,
                                            self.crop,
    ...
    def _eval_augment_img(self, img):
        return simclr_eval_augmentation(img,
                                        self.height,
                                        self.width,
                                        self.crop,

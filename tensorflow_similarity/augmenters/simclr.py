...
def random_apply(func: Callable[[Tensor], Tensor], p: float,
                 x: Tensor) -> Tensor:
...
def random_brightness(image: Tensor,
                      max_delta: float,
                      impl: str = "simclrv2") -> Tensor:
...
def color_jitter_nonrand(image: Tensor,
                         brightness: float = 0,
                         contrast: float = 0,
                         saturation: float = 0,
                         hue: float = 0,
    ...
    with tf.name_scope("distort_color"):
        ...
        def apply_transform(i: int, x: Tensor, brightness: float,
...
def color_jitter_rand(image: Tensor,
                      brightness: float = 0,
                      contrast: float = 0,
                      saturation: float = 0,
                      hue: float = 0,
...
def _compute_crop_shape(image_height: int, image_width: int,
                        aspect_ratio: float,
                        crop_proportion: float) -> Tuple[int, int]:
    ...
    def _requested_aspect_ratio_wider_than_image():
        ...
        crop_width = tf.cast(tf.math.rint(crop_proportion * image_width_float),
                             tf.int32)
...
def center_crop(image: Tensor, height: int, width: int,
                crop_proportion: float) -> Tensor:
...
def crop_and_resize(image: Tensor, height: int, width: int) -> Tensor:
    ...
    img = tf.image.resize([image], [height, width],
                          method=tf.image.ResizeMethod.BICUBIC)[0]
    return img


def gaussian_blur(image: Tensor,
                  kernel_size: Tensor,
                  sigma: float,
                  padding: str = "SAME") -> Tensor:
...
def random_crop_with_resize(image: Tensor,
                            height: int,
                            width: int,
                            p: float = 1.0) -> Tensor:
    ...
    def _transform(image: Tensor) -> Tensor:  # pylint: disable=missing-docstring
...
def random_color_jitter(image: Tensor,
                        p: float = 1.0,
                        strength: float = 1.0,
                        impl: str = "simclrv2") -> Tensor:
...
def batch_random_blur(images_list: List[Tensor],
                      height: int,
                      width: int,
                      blur_probability: float = 0.5) -> List[Tensor]:
...
def simclr_eval_augmentation(image: Tensor,
                             height: int,
                             width: int,
                             crop: bool = True,
                             crop_proportion: float = 0.875) -> Tensor:
...
class SimCLRAugmenter(Augmenter):
    ...
    @tf.function
    def augment(self, x: Tensor, y: Tensor, num_views: int,
        ...
        with tf.device("/cpu:0"):
            ...
            for _ in range(num_views):
                ...
                view = tf.map_fn(self.augment_img,
                                 inputs,
    ...
    def _eval_augment_img(self, img: Tensor) -> Tensor:
        return simclr_eval_augmentation(img, self.height, self.width, self.crop,
                                        self.eval_crop_proportion)

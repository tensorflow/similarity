...
from tensorflow_similarity.search.nmslib import NMSLib
...
def test_reset():
    ...
    idxs, dists = search_index.lookup(target, k=2)
    ...
    search_index.reset()
    # switch order
    search_index.add(embs[1], 0)
    search_index.add(embs[0], 1)
    idxs, dists = search_index.lookup(target, k=2)
...
def test_batch_vs_single(tmp_path):
    ...
    # build search_index
    search_index = NMSLib("cosine", vect_dim)

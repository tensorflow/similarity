...
def test_reduce_sum_scalars():
    ...
    assert np.array_equal(y_, [2] * BATCH_SIZE)


def test_reduce_sum_scalars_oob():
    input_shape = [3]
    x = np.ones([BATCH_SIZE] + input_shape)
    y = np.ones(BATCH_SIZE)
...
def test_reduce_sum_vectors():
    ...
    ev = np.ones((BATCH_SIZE, 5)) * 3
    assert np.array_equal(ev, y_)


def test_reduce_sum_vectors_to_scalars():
    input_shape = [3, 5]
    x = np.ones([BATCH_SIZE] + input_shape)
    y = np.ones([BATCH_SIZE])
    ...
    ev = np.ones((BATCH_SIZE)) * 15
    assert np.array_equal(ev, y_)


def test_reduce_sum_vectors_to_scalars_implicit():
    input_shape = [3, 5]
    x = np.ones([BATCH_SIZE] + input_shape)
    y = np.ones([BATCH_SIZE])
...
def test_reduce_sum_matrices():
    ...
    ev = np.ones((BATCH_SIZE, 5, 6)) * 3
    assert np.array_equal(ev, y_)


def test_reduce_sum_matrics_to_scalars():
    input_shape = [3, 5, 6]
    x = np.ones([BATCH_SIZE] + input_shape)
    y = np.ones([BATCH_SIZE])
    ...
    assert np.array_equal([BATCH_SIZE], y_.shape)
    ev = np.ones((BATCH_SIZE)) * 90
    assert np.array_equal(ev, y_)


def test_reduce_sum_matrics_to_scalars_implicit():
